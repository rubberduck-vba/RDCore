...
internal record class BinaryConcatOperatorRuntimeSemantics : BinaryOperatorRuntimeSemantics
{
    ...
    protected override void DiagnosePreLetCoerceOperands(VBExecutionContext context, VBBinaryOperatorExpression expression, VBTypedValue lhs, VBTypedValue rhs)
    {
        if (lhs is not VBStringValue and not VBNullValue)
        {
            context.AddDiagnostic(RDCoreDiagnostic.ImplicitStringCoercion(expression.Left.Location.Range, lhs.TypeInfo));
        }
        if (rhs is not VBStringValue and not VBNullValue)
        {
