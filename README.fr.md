...
| Instances prédéclarées — variables d'instance par défaut `VB_PredeclaredId` (MS-VBAL §5.2.4.1.2) | 🚧 volet statique : le nom d'une classe prédéclarée est une variable globale du type de la classe elle-même dans le contexte de liaison par défaut, et une classe non prédéclarée n'est pas une valeur du tout; l'auto-instanciation à l'exécution, la constante `Is Nothing` et `VB_GlobalNameSpace` ne sont pas encore modélisées |
