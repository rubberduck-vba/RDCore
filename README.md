...
| Predeclared instances — `VB_PredeclaredId` default instance variables (MS-VBAL §5.2.4.1.2) | 🚧 static side: a predeclared class's name is a global variable of the class's own type in the default binding context, and a class that isn't predeclared is not a value at all; run-time auto-instantiation, the constant `Is Nothing` and `VB_GlobalNameSpace` aren't modeled yet |
