...
internal record class AppTheme
{
    private static readonly Lazy<AppTheme> _default = new(() => new(), LazyThreadSafetyMode.PublicationOnly);
    public static AppTheme Default => _default.Value;
...
internal interface IThemeColorParser
{
    int ParseThemeColor(string value);
}
internal record class AppThemeModel : IThemeColorParser
{
    public AppThemeModel(AppTheme theme)
    {
        Source = theme.Source;
        Name = theme.Name;
        Author = theme.Author;
