...
/// the convenience methods cover the common "one styled line" cases.
/// </summary>
public interface IConsoleMessageWriter
{
    ...
    /// </summary>
    /// <param name="builder">The message parts (kind, optional title / timestamp / body / verbose / stack trace / placeholders).</param>
    /// <param name="color">Overrides the kind's accent colour for this message.</param>
    ...
    /// </summary>
    /// <param name="kind">The message kind, which selects the icon and accent colour.</param>
    /// <param name="message">The message body.</param>
    /// <param name="title">An optional bold title shown ahead of the body.</param>
    ...
    /// <summary>
    /// Writes <paramref name="message"/> as an <see cref="MessageKind.Information"/> message, optionally titled.
    /// </summary>
    IConsoleMessageWriter Info(string message, string? title = default) => Message(MessageKind.Information, message, title);

    /// <summary>
    /// Writes <paramref name="message"/> as a <see cref="MessageKind.Success"/> message, optionally titled.
    /// </summary>
    IConsoleMessageWriter Success(string message, string? title = default) => Message(MessageKind.Success, message, title);

    /// <summary>
    /// Writes <paramref name="message"/> as a <see cref="MessageKind.Warning"/> message, optionally titled.
    /// </summary>
    IConsoleMessageWriter Warn(string message, string? title = default) => Message(MessageKind.Warning, message, title);

    /// <summary>
    /// Writes <paramref name="message"/> as an <see cref="MessageKind.Error"/> message, optionally titled.
    /// </summary>
